...
from dashboard.pdf_generator import generate_bi_pdf
...
def bi_section(view: str, description: str, tip: str = "", key: str = ""):
    ...
    with col_dl:
        render_download_button(df, f"bi_{key or view}", "Export Register to CSV")
...
def render_bi_page():
    ...
    # Download BI Insights Document Button (Precision Engine Spec)
    st.markdown("### Report Generation")
    col_pdf, _ = st.columns([2, 2])
    with col_pdf:
        try:
            pdf_bytes = generate_bi_pdf()
            st.download_button(
                label="📥 Download Overall Business Report (PDF)",
                data=pdf_bytes,
                file_name=f"techno_kids_bi_report_{date.today()}.pdf",
                mime="application/pdf",
                use_container_width=True,
                key="dl_bi_report_pdf"
            )
        except Exception as e:
            st.error(f"Failed to generate report PDF: {e}")

    st.markdown("---")
    st.markdown("### BI Modules")

    # §1 Customer Growth
    st.markdown("#### 📈 §1 — Customer Growth")
    t1, t2, t3, t4 = st.tabs([
        "Monthly Trend",
    ...
    st.markdown("#### ⏳ §2 — Waiting List & Conversion")
    t1, t2, t3, t4 = st.tabs([
        "Waitlist Summary",
    ...
    st.markdown("#### 💰 §3 — Revenue & Collections")
    ...
    st.markdown("#### 📚 §4 — Course Performance")
    ...
    st.markdown("#### 👷 §5 — Staffing & Instructor Cost")
    t1, t2, t3, t4 = st.tabs([
        "Instructor Load",
        "Revenue Share Cost",
    ...
    st.markdown("#### 📋 §7 — Attendance & Enrollment Lifecycle")
    ...
    # §8 Weekly Schedule
    st.markdown("#### 🗓️ §8 — Weekly Schedule")
    bi_section(
