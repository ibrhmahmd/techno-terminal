...
def get_users_for_employee(employee_id: int) -> List[User]:
    with get_session() as session:
        return repo.get_users_by_employee_id(session, employee_id)


def force_reset_password(user_id: int, new_password: str) -> None:
...
def link_employee_to_new_user(
    employee_id: int, username: str, raw_password: str, role: str
) -> User:
