...
def render_employee_detail(emp_id: int):
    ...
    st.caption(f"Job Title: {emp.job_title or 'Unspecified'} | Status: {'🟢 Active' if emp.is_active else '🔴 Inactive'}")
    
    tab_info, tab_proj, tab_sys = st.tabs(["👤 Info & Edit", "📚 Projects & Schedule", "⚙️ System Account"])
    ...
    with tab_proj:
        ...
        if my_groups:
            import pandas as pd
            g_data = [{"ID": g.id, "Name": g.name, "Day": g.default_day, "Time": g.default_time_start} for g in my_groups]
            st.dataframe(pd.DataFrame(g_data), hide_index=True, use_container_width=True)
        else:
            st.info("No active groups assigned.")
            
    with tab_sys:
        users = auth_service.get_users_for_employee(emp.id)
        if users:
            u = users[0]
            ...
            with st.form(f"sys_edit_{u.id}"):
                u_role = st.selectbox("Role", ["admin", "instructor", "frontend"], index=["admin", "instructor", "frontend"].index(u.role) if u.role in ["admin", "instructor", "frontend"] else 1)
                u_active = st.checkbox("Account Active", value=u.is_active)
                if st.form_submit_button("Update Access"):
                    from app.db.connection import get_session
                    from app.modules.auth.auth_repository import update_user_and_employee
                    try:
                        with get_session() as db:
                            update_user_and_employee(db, u.id, u_active, u_role)
                            db.commit()
            ...
            with st.expander("Force Password Reset"):
                with st.form(f"pw_reset_{u.id}"):
                    ...
                    if st.form_submit_button("Reset Password", type="primary"):
                        try:
                            auth_service.force_reset_password(u.id, new_pw)
            ...
            with st.form(f"link_sys_{emp.id}"):
                ...
                if st.form_submit_button("Create Login", type="primary"):
                    try:
                        auth_service.link_employee_to_new_user(emp.id, n_user.strip(), n_pass, n_role)
