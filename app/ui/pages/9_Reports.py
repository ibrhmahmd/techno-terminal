...
with tab_fin:
    ...
    if outstanding:
        df_out = pd.DataFrame([r.model_dump() for r in outstanding])
    ...
    if debtors:
        df_debt = pd.DataFrame([r.model_dump() for r in debtors])
...
with tab_comp:
    ...
    if fee_summary:
        df_comp = pd.DataFrame([r.model_dump() for r in fee_summary])
...
with tab_heatmap:
    ...
    if not groups_h:
        ...
        if heat_data:
            df_heat = pd.DataFrame([r.model_dump() for r in heat_data])
